...
static void itimer_evt_cb(void *p)
{
	...
	evt->running = 1;
	for (; count > 0; count--) {
        if (evt->fn && evt->running) {
            evt->fn(evt->data, evt->user);
        }	else {
            break;
        }
	}
...
//下面采用C++简单封装一下便于使用
TimerMgr::TimerMgr(uint64_t curMillSec/*ms*/, uint32_t interval/*ms*/)
{
    mMgr = new itimer_mgr();
    itimer_mgr_init(mMgr, static_cast<IUINT32>(curMillSec) , interval);
}
...
TimerMgr::~TimerMgr()
{
    for(auto& iter : mEvt)
    {
        ...
        {
            itimer_evt_destroy(iter.second);
        }
    }
...
uint64_t TimerMgr::TimerEvtInit(EvtCallBack&& callBack, void* data, void* user)
{
    itimer_evt* evt = new itimer_evt();
    evt->evtId = ++mIndex;
    assert(evt->evtId != 0);
    itimer_evt_init(evt, std::forward<EvtCallBack>(callBack), data, user);
    AddEvt(evt);
    return evt->evtId;
}
...
void TimerMgr::TimerEvtStop(uint64_t evtId)
{
    itimer_evt* pEvt = FindEvt(evtId);
    if(pEvt != nullptr && mMgr != nullptr)
    {
        itimer_evt_stop(mMgr,pEvt);
    }
}

void TimerMgr::TimerEvtDestroy(uint64_t evtId)
{
    itimer_evt* pEvt = FindEvt(evtId);
    if(pEvt != nullptr)
    {
        itimer_evt_destroy(pEvt);
        RemoveEvt(evtId);
        delete pEvt;
    }
}

void TimerMgr::TimerMgrDestroy()
{
    for(auto& iter : mEvt)
    {
        if(iter.second != nullptr)
...
void TimerMgr::Run(uint64_t curMillSec)
{
    itimer_mgr_run(mMgr, static_cast<IUINT32>(curMillSec));
}

void TimerMgr::AddEvt(itimer_evt* evt)
{
    itimer_evt* pEvt = FindEvt(evt->evtId);
    if(pEvt == nullptr)
    {
        mEvt[evt->evtId] = evt;
    }
...
itimer_evt* TimerMgr::FindEvt(uint64_t evtId)
{
    const auto& iter = mEvt.find(evtId);
    if(iter != mEvt.end())
    {
        return iter->second;
    }
...
void TimerMgr::RemoveEvt(uint64_t evtId)
{
    mEvt.erase(evtId);
}
